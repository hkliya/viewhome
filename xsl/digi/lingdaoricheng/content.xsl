					...
					<div data-role="content" align="center">
						...
						<div data-role="collapsible-set" data-theme="c" data-content-theme="d">
							<ul data-role="listview" data-inset="true">
								...
								<li>
									<xsl:apply-templates select="//table[@id='tblInfo']/tbody/tr" mode="c2"/>
								</li>
								<li data-role="list-divider">日程安排</li>
								<li>
									<xsl:apply-templates select="//table[@id='tblZDList']/tbody/tr[position()&gt;1]" mode="time"/>
								</li>
							</ul>
						</div>
					</div><!-- /content -->
		...
		<xsl:if test="not(contains(@style, 'display:none'))">
			...
			<xsl:choose>
				<xsl:when test="@class='DF_GTable_LTD_Style'">
					<span style="width:38%;display:inline-block;text-align:right"><xsl:value-of select="."/></span>
				</xsl:when>
