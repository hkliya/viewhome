	...
	<xsl:template name="file">
		...
		<xsl:choose>
			<xsl:when test="contains($info, ';')">
				<a href="javascript:void(0)" onclick="viewfile('/mobile/view/oa/file/Produce/DigiFlowMobile.nsf/0/{//input[@name='AttachDocUnid']/@value}/$file/{substring-before($info, '(')}');"  data-role="button"><xsl:value-of select="substring-before($info, '(')"/></a>
				<xsl:call-template name="file">
			...
			<xsl:otherwise>
				<a href="javascript:void(0)" onclick="viewfile('/mobile/view/oa/file/Produce/DigiFlowMobile.nsf/0/{//input[@name='AttachDocUnid']/@value}/$file/{$info}');"  data-role="button"><xsl:value-of select="$info"/></a>
			</xsl:otherwise>
		</xsl:choose>
		</li>
	</xsl:template>
